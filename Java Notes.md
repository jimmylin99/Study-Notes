...
每个类都有一个Class对象（编译时生成），每个类的（常规）创建是由这个类的Class对象完成的，

###### 类加载器

JVM通过类加载器来动态加载类，首先判断类的Class对象是否被加载，如果没有加载，则通过`.class`文件的字节码创建一个Class对象

###### 类字面常量

类字面常量返回对该类Class对象的引用

`FancyToy.class` 相比于 `Class.forName("FancyToy")` 更安全，因为类字面常量会在编译时受到检查；而且更简单、高效

它可以被应用于普通的类、接口、数组以及基本数据类型

* 包装类中的`TYPE`字段是基本数据类型`.class`类字面常量的引用
